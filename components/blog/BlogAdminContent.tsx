...
export default function BlogAdminContent() {
  ...
  const filteredPosts = posts.filter((post) => {
    ...
    return (
      ...
      categories.find(cat => cat.id === post.category_id)?.name.toLowerCase().includes(searchLower)
    );
  });
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      {/* Header with Actions */}
      <div className="border-b border-gray-700 bg-[#0D1117]">
          ...
          {isLoading && posts.length === 0 ? (
            ...
            <div className="max-w-6xl mx-auto">
              <div className="mb-4 font-mono text-sm text-gray-400">
                {filteredPosts.length} {filteredPosts.length === 1 ? 'post' : 'posts'}{searchQuery && ` of ${posts.length} total`}
              </div>
